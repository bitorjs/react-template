...
setTimeout(() => {
  window.webview = React.createElement(Demo, {
    name: 'demo'
  });
  console.log('212')
}, 3000)

let ro = class extends React.Component {
  // states = { a: 1 };
  constructor(props) {
    super(props)
    this.state = {
...
const react = _interopRequireDefault(React);
console.log(react.default.Component)
...
window.root = React.createElement(ro, {
  name: 'e'
}, window.webview);
...
class RootElement extends React.Component {
  ...
  render() {
    ...
    return window.rootE ? React.Children.only(window.rootE) : react.default.cloneElement(React.createElement(Demo, {
      name: 'demo'
    }))
  }
}
...
const C = react.default.Component;

setTimeout(() => {
  window.rootE = React.createElement(ro, {
    name: 'e'
  });
  rootEle.setState({ update: true })
}, 4000)
